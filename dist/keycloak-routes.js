...
import { buildAuthorizationUrl, exchangeCode, generateOidcChecks, getEndSessionUrl, syncUserFromClaims, verifyAndDecodeIdToken, NO_ROLE_FOR_APPLICATION, } from "./keycloak-auth.js";
function getOidcSession(req) {
    if (!req.session.oidc)
        req.session.oidc = {};
...
export function registerKeycloakAuthRoutes(app, deps) {
    ...
    app.get("/api/auth/callback", async (req, res) => {
        ...
        try {
            const tokenSet = await exchangeCode(req, expectedState, expectedNonce, deps);
    ...
    app.post("/api/auth/logout", async (req, res) => {
        ...
        const tenantIdAtLogout = req.user?.tenantId ??
            resolveTenantId();
        const ipAddress = req.ip || req.socket.remoteAddress || null;
        const oidc = getOidcSession(req);
    ...
    app.get("/api/users", deps.requireAuth, async (_req, res) => {
        try {
            ...
            res.json(allUsers.map((u) => serializeUserForClient(u)));
        }
        catch (err) {
...
//# sourceMappingURL=keycloak-routes.js.map