...
function requireEnv(name: string): string {
  ...
  if (!v || v.trim().length === 0) {
    throw new Error(
      `[keycloak] Missing required env var: ${name}. Required when AUTH_MODE=keycloak.`,
    );
  }
  return v.trim();
}

export function getCallbackUrl(deps?: { isDevMode?: () => boolean }): string {
  ...
  if (deps?.isDevMode?.()) return "http://localhost:5000/api/auth/callback";
  throw new Error(
    "[keycloak] Cannot derive callback URL. Set KEYCLOAK_CALLBACK_URL or APP_BASE_URL.",
  );
}

function getPostLogoutRedirectUri(deps?: { isDevMode?: () => boolean }): string {
  ...
  if (deps?.isDevMode?.()) return "http://localhost:5000/";
  throw new Error(
    "[keycloak] Cannot derive post-logout redirect URI. Set KEYCLOAK_POST_LOGOUT_REDIRECT_URI or APP_BASE_URL.",
  );
}
...
export function resetKeycloakClientForTests(): void {
  cachedClient = null;
  cachedIssuer = null;
  cachedJwks = null;
}
...
async function getJwks(deps?: {
  ...
  if (!jwksUri) throw new Error("[keycloak] Issuer metadata is missing jwks_uri");
...
export function generateOidcChecks(): OidcAuthParams {
 ...
 */
export const KEYCLOAK_TO_APP_ROLE: Readonly<Record<string, string>> = Object.freeze({
  ciso: "CISO",
 ...
 */
export function mapClaimsToAppRole(
  realmRoles: ReadonlyArray<string>,
  keycloakToApp: Record<string, string> = KEYCLOAK_TO_APP_ROLE,
  validRoles: readonly string[] = Object.values(KEYCLOAK_TO_APP_ROLE),
): string {
  ...
  for (const r of realmRoles) {
    ...
    if (mapped && validRoles.includes(mapped)) return mapped;
  }
  throw new Error(
    `[keycloak] No mappable role found in realm roles: ${realmRoles.join(", ")}`,
  );
}
 ...
 */
export function extractAppRole(
  jwt: JWTPayload | KeycloakClaims,
  clientId: string,
  keycloakToApp: Record<string, string>,
): string | null {
  if (!jwt || typeof jwt !== "object") return null;
  const ra = (jwt as KeycloakClaims).resource_access;
...
export async function syncUserFromClaims(
  ...
  if (existing) {
    ...
    const patch: UpdateUserInput = { lastLoginAt: new Date() };
    if (!existing.keycloakId) patch.keycloakId = claims.sub;
...
export function makeRequireAuthKeycloak(validRoles: readonly string[]) {
  return function requireAuthKeycloak(
    req: Request,
    res: Response,
    next: NextFunction,
  ) {
...
export function makeRequireRoleKeycloak(validRoles: readonly string[]) {
  return function requireRoleKeycloak(...roles: string[]) {
